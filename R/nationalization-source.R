...
nationalization <- function(dataSource, dataType, dataQuality,
  ...
  gini.ineq <- setDT(gini.small_removed)[,giniI := ineq(vote.shares, NULL,type = inequalityType, na.rm = TRUE),by=c("ctr_n","ctr","yr","mn","pty")] # Removed id
  ...
  pty.unique_rows <- !duplicated(data.a[c("id","ctr_n","ctr","yr", "mn","cst","pty")])
  pty.unique <- data.a[pty.unique_rows,]
  pty.base <- pty.unique
  
  # Merging in gini, then using it to compute vanilla PNS
  pty.gini <- merge(pty.base, full.gini, by = c("id","ctr_n","ctr","yr","mn","cst","pty","vv1"),all.x = TRUE)
  ...
  pty.cst_tot_unique_rows <- !duplicated(pty.cst_tot[c("id","ctr_n","ctr","yr", "mn","cst","pty")])
  pty.unique_cst_tot <- pty.cst_tot[pty.cst_tot_unique_rows,]
  pty <- pty.unique_cst_tot
  
  # Grabbing nat_vv1, pty_vv1; reassigning to pty
  pty.nat_vv1 <- data.table(pty)[,nat_vv1 := sum(vote.totals, na.rm = TRUE), by=c("ctr_n","ctr","yr","mn")] # Removed id
  pty.pty_vv1 <- data.table(pty.nat_vv1)[,pty_vv1 := sum(pv1, na.rm = TRUE), by=c("ctr_n","ctr","yr","mn","pty")] # Removed id
  ...
  pty.p_j <- data.table(pty.ordered)[,p_j := cumsum(vote.totals), by=c("ctr_n","ctr","yr","mn","pty")] # Removed id

  # Computing the "inside" of the numerator's sum, then computing PNS_w numerator; reassigning to "party"
  pty.inside <- data.table(pty.p_j)[,inside := cst_vv1 * (p_j - (vote.totals / 2)), by=c("ctr_n","ctr","yr","mn","pty")] # Removed id  #inside = cst_vv1 * (p_j - (pv1/2))
  pty.numerator <- data.table(pty.inside)[,numerator := sum(inside, na.rm=TRUE),by=c("ctr_n","ctr","yr","mn","pty")] # Removed id

  # Computing PNS_w at long last! Replacing infinite/NaN with NA, then reassign to "party" 
  pty.pns_w <- data.table(pty.numerator)[,PNS_w := ((2 * numerator)/ denominator),by=c("ctr_n","ctr","yr","mn","pty")] # Removed id
  ...
  pty.pj_new <- data.table(pty.pns_w)[,p_j_new := cumsum(vote.totals), by=c("ctr_n","ctr","yr","mn","pty")] # Removed id
  pty.inside_new <- data.table(pty.pj_new)[,inside_new := cst_vv1_new * (p_j_new - (vote.totals / 2)), by=c("ctr_n","ctr","yr","mn","pty")] # Removed id
  pty.numerator_new <- data.table(pty.inside_new)[,numerator_new := sum(inside_new, na.rm=TRUE),by=c("ctr_n","ctr","yr","mn","pty")] # Removed id

  pty.pns_w_new <- data.table(pty.numerator_new)[,PNS_w_new := ((2 * numerator_new) / denominator), by=c("ctr_n","ctr","yr","mn","pty")] # Removed id
  ...
  pty.pid <- data.table(pty.slim)[,pid := 1:length(nat_vv1),by=c("ctr_n","ctr","yr","mn","cst")] # Removed id
  ...
  pty.bottom <- data.table(pty.pid)[,bottom := sum(helper, na.rm=TRUE), by=c("ctr_n","ctr","yr")] # Removed id
  pty.bottom$power_E <- pty.bottom$top / pty.bottom$bottom
  
  pty.pns_sw <- data.table(pty.bottom)[,PNS_sw := (PNS_w)^(1/(log10(power_E))), by=c("ctr_n","ctr","yr")] # Removed id
  ...
  pty.slimmer <- subset(pty.to_clean, select = c(id, ctr_n, ctr, yr, mn, pty_n, pty, PNS, PNS_s, PNS_w, PNS_sw, cst_tot))
  full.pty <- pty.slimmer
  
  full.pty <- as.data.frame(full.pty[order(full.pty$ctr_n,full.pty$yr,full.pty$pty),])
  pty.full_rows <- !duplicated(full.pty[c("id","ctr_n","ctr","yr", "mn","pty")])
  pty.full_unique <- full.pty[pty.full_rows,]
  full.pty <- pty.full_unique
  ...
  enp.totals <- data.table(data.b)[,vote.totals := sum(vote.totals, na.rm = TRUE), by=c("ctr_n","ctr","yr","mn","pty")] # Removed id
  
  # Grabbing unique again to get the data to the national level
  enp.totals <- as.data.frame(enp.totals)
  enp.unique.rows <- !duplicated(enp.totals[c("id","ctr_n","ctr","yr", "mn","pty")])
  enp.unique <- enp.totals[enp.unique.rows,]

  # Computing ENP_nat, then selecting down to relevant columns
  enp.nat_vv1 <- data.table(enp.unique)[,nat_vv1 := sum(vote.totals, na.rm = TRUE), by=c("ctr_n","ctr","yr","mn")] # Removed id
  enp.shares_sq <- data.table(enp.nat_vv1)[,party_prop_nat2 := (vote.totals/nat_vv1)^2, by=c("ctr_n","ctr","yr","mn")] # Removed id
  enp.denom <- data.table(enp.shares_sq)[,denominator := sum(party_prop_nat2, na.rm = TRUE), by=c("ctr_n","ctr","yr","mn")] # Removed id
  enp.enep_nat <- data.table(enp.denom)[,ENEP_nat := 1/denominator, by=c("ctr_n","ctr","yr","mn")] # Removed id
  enp.enep_nat <- subset(enp.enep_nat, select = c(id,ctr_n,ctr,yr,mn,ENEP_nat,cst))
  
  # Computing ENP_cst (using same unique data above)
  enp.new_vv1 <- data.table(enp.unique)[,new_vv1 := sum(vote.totals),by=c("ctr_n","ctr","yr","mn","cst")]
  enp.new_shares_sq <- data.table(enp.new_vv1)[,share_sq := (vote.totals/new_vv1)^2,by=c("ctr_n","ctr","yr","mn","cst")]
  e.new_denom <- data.table(enp.new_shares_sq)[,denominator := sum(share_sq),by=c("ctr_n","ctr","yr","mn","cst")]
  enp.enep_cst <- data.table(e.new_denom)[,ENEP_cst := 1/denominator,by=c("ctr_n","ctr","yr","mn","cst")]
  ...
  enp.unique.rows_enp <- !duplicated(enp.enep_cst[c("id","ctr_n","ctr","yr", "mn","cst","pty")])
  ...
  enp.new_nat_vv1 <- data.table(enp.new_indicator)[,nat_vv1 := sum(new_vv1, na.rm=TRUE),by=c("ctr_n","ctr","yr","mn")]
  enp.cst_wght <- data.table(enp.new_nat_vv1)[,cst_wght := (new_vv1/nat_vv1),by=c("ctr_n","ctr","yr","mn")]
  enp.weighted <- data.table(enp.cst_wght)[,weighted := (cst_wght * ENEP_cst),by=c("ctr_n","ctr","yr","mn")]
  enp.enep_wght <- data.table(enp.weighted)[,ENEP_wght := sum(weighted),by=c("ctr_n","ctr","yr","mn")]
  
  # Now, we compute average ENP (ENP_avg) real quick!
  enp.enep_avg <- data.table(enp.enep_wght)[,ENEP_avg := mean(ENEP_cst),by=c("ctr_n","ctr","yr","mn")]
  enp.enp_avg_wght <- as.data.frame(enp.enep_avg)
  
  # Arranging both ENP-related sets, combining them, then cleaning them up
  enp.merge <- merge.data.frame(enp.enep_nat,enp.enp_avg_wght,by = c("id","ctr_n","ctr","yr","mn","cst"))
  
  enp.slim <- subset(enp.merge, select = c(id,ctr_n,ctr,yr,mn,ENEP_nat,ENEP_avg,ENEP_wght,indicator))
  enp.slim_rows <- !duplicated(enp.slim[c("id","ctr_n","ctr","yr", "mn")])
  e.full <- enp.slim[enp.slim_rows,]
  full.enp <- e.full[order(e.full$ctr_n,e.full$yr,e.full$mn),]
  ...
  nat.unique.rows <- !duplicated(data.c[c("id","ctr_n","ctr","yr", "mn","cst","pty")])
  unique.nat <- data.c[nat.unique.rows,]
  data.c <- unique.nat
  
  # Computing ENP_cst (using unique data above)
  psns.new_vv1 <- data.table(data.c)[,new_vv1 := sum(vote.totals),by=c("ctr_n","ctr","yr","mn","cst")]
  psns.new_shares_sq <- data.table(psns.new_vv1)[,share_sq := (vote.totals/new_vv1)^2,by=c("ctr_n","ctr","yr","mn","cst")]
  psns.new_denom <- data.table(psns.new_shares_sq)[,denominator := sum(share_sq, na.rm = TRUE),by=c("ctr_n","ctr","yr","mn","cst")]
  psns.enep_cst <- data.table(psns.new_denom)[,ENEP_cst := 1/denominator,by=c("ctr_n","ctr","yr","mn","cst")]
  
  # Merging this ENP_cst with the full ENP above at the national-level
  psns.full_enp <- merge(psns.enep_cst,full.enp, by = c("id","ctr_n","ctr","yr","mn"))

  # Renaming to just "psns" for easier typing, then computing prerequisites for inflation values (MK_N)
  psns <- psns.full_enp
  ...
  psns.nat_vote <- data.table(psns)[, nat_vote := sum(new_vv1, na.rm = TRUE), by = c("ctr_n","ctr","yr","mn")]
  psns.cst_vote_prop <- data.table(psns.nat_vote)[,cst_vote_prop := new_vv1/nat_vote, by = c("ctr_n","ctr","yr","mn")]
  psns.product <- data.table(psns.cst_vote_prop)[,product := ENEP_cst *  cst_vote_prop, by = c("ctr_n","ctr","yr","mn")]
  psns.sum_cst <- data.table(psns.product)[,sum_cst := sum(product, na.rm = TRUE), by = c("ctr_n","ctr","yr","mn")]
  psns.denom <- data.table(psns.sum_cst)[,denominator := cst_tot * sum_cst, by = c("ctr_n","ctr","yr","mn")]
  psns.W_tilde <- data.table(psns.denom)[,W_tilde := ENEP_cst/ denominator, by = c("ctr_n","ctr","yr","mn")]
  psns.I_w <- data.table(psns.W_tilde)[,I_w := ((ENEP_nat - sum_cst)/sum_cst)*100, by = c("ctr_n","ctr","yr","mn")]
  psns.numerator <- data.table(psns.I_w)[,numerator := (I_i -  I_w)^2 * W_tilde, by = c("ctr_n","ctr","yr","mn")]
  psns.sum_numerator <- data.table(psns.numerator)[,sum_numerator := sum(numerator, na.rm = TRUE), by = c("ctr_n","ctr","yr","mn")]
  psns.coeff_var_I_i <- data.table(psns.sum_numerator)[,coeff_var_I_i := sqrt(sum_numerator)/I_w, by = c("ctr_n","ctr","yr","mn")]
  psns.numerator_r2 <- data.table(psns.coeff_var_I_i)[,numerator_r2 := ((I_i - I_w)^4) * W_tilde, by = c("ctr_n","ctr","yr","mn")]
  psns.sum_numerator_r2 <- data.table(psns.numerator_r2)[,sum_numerator_r2 := sum(numerator_r2, na.rm = TRUE), by = c("ctr_n","ctr","yr","mn")]
  psns.denominator_r2 <- data.table(psns.sum_numerator_r2)[,denominator2 := ((I_i - I_w)^2) * W_tilde, by = c("ctr_n","ctr","yr","mn")]
  psns.sum_denominator_r2 <- data.table(psns.denominator_r2)[,sum_denominator_r2 := sum(denominator2, na.rm=TRUE), by = c("ctr_n","ctr","yr","mn")]
  psns.sum_denominator_r2$sq_sum_denominator_r2 <- (psns.sum_denominator_r2$sum_denominator_r2)^2
  psns.kurtosis_I_i <- data.table(psns.sum_denominator_r2)[,kurtosis_I_i := sum_numerator_r2/sq_sum_denominator_r2, by = c("ctr_n","ctr","yr","mn")]
  ...
  psns.unique.rows <- !duplicated(psns.pty_vv1[c("id","ctr_n","ctr","yr", "mn","pty")])
  psns.unique <- psns.pty_vv1[psns.unique.rows,]
  
  psns.pty_merge <- merge(psns.unique,full.pty,by = c("id", "ctr_n", "ctr", "yr", "mn", "pty", "pty_n", "cst_tot"),all.x = TRUE)
  ...
  psns.psns <- data.table(psns.pty_merge)[, PSNS := sum(PNS * weight, na.rm = TRUE), by = c("ctr_n","ctr","yr","mn")]
  psns.psns_s <- data.table(psns.psns)[, PSNS_s := sum(PNS_s * weight, na.rm=TRUE), by = c("ctr_n","ctr","yr","mn")]
  psns.psns_w <- data.table(psns.psns_s)[, PSNS_w := sum(PNS_w * weight, na.rm=TRUE), by = c("ctr_n","ctr","yr","mn")]
  psns.psns_sw <- data.table(psns.psns_w)[, PSNS_sw := sum(PNS_sw *weight, na.rm=TRUE), by = c("ctr_n","ctr","yr","mn")]
  ...
  psns.slim <- subset(psns.to_edit, select = c(id,ctr_n,ctr,yr,mn,PSNS,PSNS_s,PSNS_w,PSNS_sw,MK_N,MK_N_two,cst_tot))
  ...
  local.unique_rows <- !duplicated(data.d[c("id","ctr_n","ctr","yr", "mn","cst","pty")])
  local.unique <- data.d[local.unique_rows,]
  data.d <- local.unique
  
  # Renaming to "local." plus the variable/data being created
  local.nat_vote <- data.table(data.d)[, nat_vote := sum(vote.totals, na.rm=TRUE), by = c("ctr_n","ctr","yr", "mn")]
  local.seat_cst <- data.table(local.nat_vote)[, seat_cst := sum(seat, na.rm=TRUE), by = c("ctr_n","ctr","yr", "mn","cst")]
  local.party_vote_prop <- data.table(local.seat_cst)[, party_vote_prop := vote.totals/nat_vote, by = c("ctr_n","ctr","yr", "mn","cst")]
  
  # Arranging, then computiong seat proportion & party vote proportion
  ...
  local.seat_total <- data.table(local.party_vote_prop)[, seat_total := sum(seat, na.rm=TRUE), by = c("ctr_n","ctr","yr", "mn")]
  local.seat_contest <- data.table(local.seat_total)[, seat_contest := sum(seat_cst, na.rm=TRUE), by = c("ctr_n","ctr","yr", "mn","pty")]
  local.seat_prop <- data.table(local.seat_contest)[, seat_prop := seat_contest/seat_total, by = c("ctr_n","ctr","yr", "mn","pty")]
  
  # Computing local_E by party, then getting national sum
  local.local_E <- data.table(local.seat_prop)[, local_E := party_vote_prop * seat_prop, by = c("ctr_n","ctr","yr", "mn","pty")]
  local.local_E_r2 <- data.table(local.local_E)[, local_E := sum(local_E, na.rm=TRUE), by = c("ctr_n","ctr","yr", "mn")]
  
  # Selecting down to relevant, then grabbing distinct
  local.local_E_r2 <- subset(local.local_E_r2, select = c(id,ctr_n,ctr,yr,mn,local_E))
  local.local_E_r2 <- as.data.frame(local.local_E_r2)
  local_E_rows <- !duplicated(local.local_E_r2[c("id","ctr_n","ctr","yr", "mn")])
  ...
  nat.enp_psns_merge <- merge(full.enp,full.psns,by = c("id","ctr_n","ctr","yr","mn"), 
                        all = TRUE, sort = TRUE)
  nat.full <- merge(nat.enp_psns_merge,full.local_E, by = c("id","ctr_n","ctr","yr","mn"), all = TRUE)
  ...
  grid <- as.data.frame(subset(base, select = c(id,ctr_n,ctr,yr,mn,cst,cst_n,pty,pty_n)))
  full.nat <- merge(nat.full,grid, by = c("id","ctr_n","ctr","yr","mn"), all = TRUE)

  # Grabbing unique, ordering, renaming ENP, then shipping!
  full.nat_rows <- !duplicated(full.nat[c("id","ctr_n","ctr","yr", "mn")])
  full.nat <- full.nat[full.nat_rows,]
  
  full.nat <- full.nat[order(full.nat$ctr_n,full.nat$yr),]
  ...
  full.nat <- subset(full.nat, select = c(id, ctr_n, ctr, yr, mn, nvvi, ENP_nat, ENP_avg, ENP_wght, inflation1, inflation2, 
  ...
  cst.unique_rows <- !duplicated(data.b[c("id","ctr_n","ctr","yr", "mn","cst","pty")])
  cst.unique <- data.b[cst.unique_rows,]
  cst.base <- cst.unique
  
  # Computing prerequisites for ENP_cst
  cst.cst_tot <- data.table(cst.base)[, cst_tot := length(unique(cst)), by = c("ctr_n","ctr","yr", "mn")]
  cst.new_vv1 <- data.table(cst.cst_tot)[, new_vv1 := sum(vote.totals, na.rm = TRUE), by = c("ctr_n","ctr","yr", "mn","cst")]
  cst.share_sq <- data.table(cst.new_vv1)[, share_sq := (vote.totals/new_vv1)^2, by = c("ctr_n","ctr","yr", "mn","cst")]
  cst.denom <- data.table(cst.share_sq)[, denom := sum(share_sq), by = c("ctr_n","ctr","yr", "mn","cst")]
  
  # Computing ENP_cst
  cst.enp_cst <- data.table(cst.denom)[, ENP_cst := 1/denom, by = c("ctr_n","ctr","yr", "mn","cst")]
  ...
  cst.enp_cst <- subset(cst.enp_cst, select = c(id,ctr_n,ctr,yr,mn,cst,cst_n,ENP_cst,cvvi))
  
  cst.enp_cst <- as.data.frame(cst.enp_cst)
  cst.unique_rows_enp <- !duplicated(cst.enp_cst[c("id","ctr_n","ctr","yr", "mn","cst")])
  cst.unique_enp <- cst.enp_cst[cst.unique_rows_enp,]
  cst.enp <- cst.unique_enp
  
  # Merging in national-level to compute inflation5
  cst.nat <- merge(cst.enp,full.enp, by = c("id","ctr_n","ctr","yr", "mn"), all = TRUE)
  ...
  cst.nat.pty <- merge(cst.nat,grid,by = c("id","ctr_n","ctr","yr", "mn","cst","cst_n"), all = TRUE)
  
  # Selecting down to relevant columns, arranging, grabbing distinct, then shipping!
  cst.slim <- subset(cst.nat.pty, select = c(id,ctr_n,ctr,yr,mn,cst,cst_n,cvvi,ENP_cst))
  cst.slim <- cst.slim[order(cst.slim$ctr_n,cst.slim$yr,cst.slim$mn,cst.slim$cst),]
  
  cst.slim_unique_rows <- !duplicated(cst.slim[c("id","ctr_n","ctr","yr", "mn","cst")])
